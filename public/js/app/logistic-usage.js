if(document.getElementById("logistic-usage-app") != null){   
        ...
        var genAccount = function(){
            var select = document.createElement("select");
            select.setAttribute('name','account_id[]');
            select.setAttribute('class','form-control');
            
            var accounts = JSON.parse(document.getElementById('listAccounts').value);
            accounts.forEach(function(acc){
                var opt = document.createElement('option'); 
                opt.innerHTML = acc.code+" | "+acc.name;
                opt.value = acc.id;
                select.appendChild(opt);
            });
            return select.outerHTML;
