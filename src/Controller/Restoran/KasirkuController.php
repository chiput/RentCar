...
class KasirkuController extends Controller
{
    public function __invoke(Request $request, Response $response, Array $args)
    {

        $data['app_profile'] = $this->app_profile;
        $data['Menus'] = Reskasirku::orderby('id','desc')->where('resto','=',1)->where('reskasirku.users_id','=',$this->session->get('activeUser')["id"])->get();

        return $this->renderer->render($response, 'restoran/kasirku', $data);


    }

    public function GenericNoBukti($NoBukti){
        $repalgs= $this->db->table('reskasirku')->where('resto','=',1)->get();
        if ($NoBukti<10){
        ...
        $status=true;
         foreach ($repalgs as $reskasir ) {
            if(strtolower($reskasir->nobukti)==strtolower("RI.".date('y').date('m').$NoBuktitext)){
                //$NoBukti++;
                $status=false;
            }
        }
        if(!$status){
           return $this->GenericNoBukti($NoBukti);
            ...
            return "RI.".date('y').date('m').$NoBuktitext ;
        }
    }
    ...
    public function form(Request $request, Response $response, Array $args)
    {
        ...
        if (isset($args['id'])){
            ...
            $data['NoBukti'] =$this->GenericNoBukti(count($this->db->table('reskasirku')->where('resto','=',1)->get()));
        }
        ...
        //$data['Rooms']=  Capsule::select("select C.*,(select name from guests where id=A.guests_id) as namapengunjung,B.id as reservationdetails_id  from reservations A,reservationdetails B,rooms C where  A.id=B.reservations_id and C.id=B.rooms_id and B.checkin_code is not null and B.checkout_at is null");
        ...
        return $this->renderer->render($response, 'restoran/kasirku-form', $data);
    }
    ...
    public function save(Request $request, Response $response, Array $args)
    {
        ...
        if (!$this->validation->passes()) {
            ...
            if ($postData['id'] == '') {
                return $response->withRedirect($this->router->pathFor('kasirku-add'));
            } else {
                return $response->withRedirect($this->router->pathFor('kasirku-edit',['id' => $postData['id']]));
            }
        }
        ...
        if (preg_match('/[^-_@. 0-9A-Za-z]/',$postData['meja'])) { 
            ...
            if($postData['zid'] == ""){
                ...
                return $response->withRedirect($this->router->pathFor('kasirku-edit',['id'=>$postData['zid']]));
            }
        }
        ...
        $setgud = Setupgudang::where('name','gud_restorente')->first();
        //Pemakaian Barang
        ...
        if ($postData['checks']=="0"){
            ...
            //    $reskasirku->kembalian = FormaterAdapter::reverse($postData['kembalian']) - FormaterAdapter::reverse($postData['kembalian']);
            // }else{
            //     $reskasirku->kembalian = FormaterAdapter::reverse($postData['kembalian']);
            // }
            $reskasirku->users_id = $this->session->get('activeUser')["id"];
        ...
        return $response->withRedirect($this->router->pathFor('kasirku-report',['id' => $insertedId ]));
    ...
    public function delete(Request $request, Response $response, Array $args)
    {
        ...
        return $response->withRedirect($this->router->pathFor('kasirku'));
    }
    ...
    private function post_jurnal($postData){
        $jurnal=[
                ...
                "keterangan" => "Penjualan Restorante Italia",
                "details"=>[]
            ];
    ...
    public function rekappenjualan(Request $request, Response $response, $args)
    {
        ...
        $data['menus'] = Reskasirku::where('tanggal','=',$this->convert_date($date))->where('reskasirku.users_id','=',$this->session->get('activeUser')["id"])->where('resto','=',1)->get();
    ...
    {
        $data['options'] = Options::all();
        $date=$this->convert_date($args['date']);
