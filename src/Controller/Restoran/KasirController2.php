...
class KasirController2 extends Controller
{
    public function __invoke(Request $request, Response $response, Array $args)
    {
        ...
        $data['NoBukti'] =$this->GenericNoBukti(count($this->db->table('reskasir')->get()));
          ...
          $data['Rooms']=  Capsule::select("select C.* from reservations A,reservationdetails B,rooms C where  A.id=B.reservations_id and C.id=B.rooms_id and B.checkin_code is not null group by C.id");
