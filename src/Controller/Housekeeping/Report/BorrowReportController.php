...
class BorrowReportController extends Controller
{
    ...
    {
        $data['submit_form'] = $this->router->pathFor('housekeeping-report-pinjambarang');

        return $this->renderer->render($response, 'housekeeping/report/borrowitem-filter', $data);
    }
    public function display(Request $request, Response $response, $args)
    {
        ...
        $data['menu'] = Hotpinjam::whereBetween('tanggal', array($start,$end))
                                    ->get();
        ///// get option ///
        $opt = Option::all();
        $Options=[];
       foreach ($opt as $value) {
        ...
        return $this->renderer->render($response, 'housekeeping/report/borrowitem', $data);
    }
}
